...
- Git 基线位于本地 `main`，HEAD 为 `d27c103`，该提交已包含此前运行时、配置和文档
  修正。本轮 2026-08-21 只更新交接文档和搜索规则，未修改算法、配置或测试；这些本地
  文档修改在审阅完成前不要清理或覆盖；
...
本轮文档更新开始前，`~/ws/src/semantic_mapping` 位于本地 `main`，基线 HEAD 为
`d27c1032f97d8e744c3ee2f2ef196c00ea6bac7e`，工作区干净。该提交已包含 CameraInfo/
投影失败关闭、Lite3 速度链、Nav2 里程计配置、离线烟测工具和 CARLA 诊断记录。

2026-08-21 的本地修改只用于把 README、PROJECT_STATUS、LITE3_REAL_HANDOFF、RUNBOOK、
AGENTS、MANIFEST 和默认搜索规则同步到最新证据；未修改算法源码、正式 YAML 或测试。
在用户审阅并决定是否提交前，不要覆盖或清理这些文档修改。

...
class GABsvmNode(Node):
    def __init__(self):
        ...
        self.voxel_map = VoxelMap(voxel_size=self.voxel_size, K=self.num_classes)
        self.get_logger().info(f'贝叶斯 VoxelMap 已创建，网格精度: {self.voxel_size:.3f}m')
    ...
    def find_approach_goal(self, object_key, object_pos, query_class_idx):
        if query_class_idx is None:
            return object_key, object_pos

        if self.semantic_cost_dict.get(query_class_idx, 0) < 100:
            return object_key, object_pos

        radius_vox = max(1, int(np.ceil(self.query_approach_radius_m / self.voxel_size)))
        ox, oy, oz = object_key
        best_key = None
        best_pos = None
        best_dist = float('inf')
        ...
        for dx in range(-radius_vox, radius_vox + 1):
            for dy in range(-radius_vox, radius_vox + 1):
                key = (ox + dx, oy + dy, oz)
                voxel = self.voxel_map.voxels.get(key)
                if voxel is None or voxel['weight_sum'] < self.query_min_weight_sum:
                    continue
                ...
                pos = voxel['pos']
                dist = np.linalg.norm(pos[:2] - object_pos[:2])
                if dist < best_dist:
                    best_dist = dist
                    best_key = key
                    best_pos = pos
        ...
        if best_pos is not None:
            self.get_logger().info(
                f'目标是障碍类，导航点改为附近 road: x={best_pos[0]:.2f}, y={best_pos[1]:.2f}')
            return best_key, best_pos
    ...
    def query_feature_cb(self, msg):
        ...
        used_fallback = False
        if best_pos is None and self.query_allow_feature_fallback and fallback_pos is not None:
            used_fallback = True
            best_score = fallback_sim
        ...
        if best_pos is not None:
            goal_key, goal_pos = self.find_approach_goal(best_key, best_pos, query_class_idx)
            ...
            self.get_logger().info(
                ...
                f'score={best_score:.3f}, sim={best_sim:.3f}, class_prob={best_class_prob:.3f}, '
                f'目标坐标: x={best_pos[0]:.2f}, y={best_pos[1]:.2f}, '
    ...
    def publish_semantic_costmap(self):
        """
        🌟 Day 61-62: 把 3D 语义体素地图拍扁成 2D 代价地图，强插 Nav2 大脑。
        固定 50m x 50m 网格 (避免 StaticLayer 频繁 resizeMap)，1Hz 发布。
        """
        if not self.voxel_map.voxels:
            return
        ...
        for key, voxel in self.voxel_map.voxels.items():
            ...
            ix, iy, iz = key
            best_class = int(np.argmax(voxel['log_odds']))

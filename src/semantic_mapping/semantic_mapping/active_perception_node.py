...
class ActivePerceptionNode(Node):
    ...
    def cmd_vel_cb(self, msg):
        ...
        angular_scale = 1.0
        if self.scale_angular_velocity:
            angular_scale = max(alpha, self.angular_min_speed_ratio)
        mod_cmd.angular.z = msg.angular.z * angular_scale

        if math.hypot(mod_cmd.linear.x, mod_cmd.linear.y) < self.linear_deadband:
            mod_cmd.linear.x = 0.0
            mod_cmd.linear.y = 0.0
        if abs(mod_cmd.angular.z) < self.angular_deadband:
            mod_cmd.angular.z = 0.0

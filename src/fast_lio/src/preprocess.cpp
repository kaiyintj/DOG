...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  float min_offset_time = std::numeric_limits<float>::max();
  float max_offset_time = -std::numeric_limits<float>::max();
  for (const auto &pt : pl_orig.points)
  {
    min_offset_time = std::min(min_offset_time, pt.time);
    max_offset_time = std::max(max_offset_time, pt.time);
  }
  const float offset_time_shift = min_offset_time < 0.0f ? -min_offset_time : 0.0f;

  if (max_offset_time > 0.0f || min_offset_time < 0.0f)
  {
  ...
  if (feature_enabled)
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature = (pl_orig.points[i].time + offset_time_shift) * time_unit_scale;  // units: ms
  ...
  else
  {
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature =
          (pl_orig.points[i].time + offset_time_shift) * time_unit_scale;  // curvature unit: ms

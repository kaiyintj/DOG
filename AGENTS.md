...
算法与实验基线 commit 为
`d27c1032f97d8e744c3ee2f2ef196c00ea6bac7e`。2026-08-21 的后续文档提交只同步
Lite3 Bag/离线烟测证据、交接入口和默认搜索规则，不改变算法源码、正式 YAML、测试或
既有实验结论。

用户已审阅并授权本轮一次性本地文档提交，以及移除与时间戳副本逐字节相同的
`refine-logs/P1_OFFLINE_DIAGNOSTIC_TRACE.md` alias；规范审计副本
`P1_OFFLINE_DIAGNOSTIC_TRACE_20260817_135410.md` 必须保留。该授权不包括删除或移动其他
历史文件，也不包括推送远端或后续提交。

历史审查、时间戳 plan/tracker、规范 P1 trace 与 `.aris` 记录仍受 Git 跟踪，只通过
`.rgignore` 从默认全文搜索中排除。执行任何新任务前必须重新记录
`git status --short --branch`；完成本轮提交后的预期工作区是干净状态，任何新增 dirty
内容均应先视为用户修改并保护。

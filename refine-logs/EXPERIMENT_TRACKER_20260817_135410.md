...
| --- | --- | --- | --- | --- | --- | --- |
| P1-OD-SETUP-001 | P0-A/B PASS | TODO | 三 manifest；YAML副本 | 计划 §16.2 | `configs/p1_offline_diagnostic/*.yaml`, run record | S：100/100/60帧、Town10/seed42/schema/模型可读 |
| P1-OD-PT-RANGE-10 | SETUP | TODO | stationary；range=10,temp=1,offset0 | §16.3 point command | `p1_offline_diagnostic/P1-OD-PT-RANGE-10/` | S：alignment/output；Q：r_range AUROC/AURC/gap |
| P1-OD-PT-RANGE-30 | SETUP | TODO | stationary；range=30,temp=1,offset0 | 同上 | `.../P1-OD-PT-RANGE-30/` | 同上；Q失败不传播 |
| P1-OD-PT-DENSITY-4 | SETUP | TODO | stationary；density=4,temp=1,offset0 | 同上 | `.../P1-OD-PT-DENSITY-4/` | S；Q：r_density；非物理密度因果 |
| P1-OD-PT-DENSITY-16 | SETUP | TODO | stationary；density=16,temp=1,offset0 | 同上 | `.../P1-OD-PT-DENSITY-16/` | 同上 |
| P1-OD-PT-VIEW-0P2 | SETUP | TODO | stationary；view=.2,temp=1,offset0 | 同上 | `.../P1-OD-PT-VIEW-0P2/` | S；Q：r_view AUROC/AURC/gap |
| P1-OD-PT-VIEW-0P4 | SETUP | TODO | stationary；view=.4,temp=1,offset0 | 同上 | `.../P1-OD-PT-VIEW-0P4/` | 同上 |
| P1-OD-PT-SEMFLOOR-0P05 | SETUP | TODO | stationary；floor=.05,temp=1,offset0 | 同上 | `.../P1-OD-PT-SEMFLOOR-0P05/` | S；Q：r_semantic AUROC/AURC/gap |
| P1-OD-PT-SEMFLOOR-0P30 | SETUP | TODO | stationary；floor=.30,temp=1,offset0 | 同上 | `.../P1-OD-PT-SEMFLOOR-0P30/` | 同上 |
| P1-OD-PT-TEMP-0P75 | SETUP | TODO | stationary；base YAML,temp=.75,offset0 | 同上 | `.../P1-OD-PT-TEMP-0P75/` | S；Q：accuracy/ECE/NLL/Brier/r_semantic |
| P1-OD-PT-TEMP-1P50 | SETUP | TODO | stationary；base YAML,temp=1.5,offset0 | 同上 | `.../P1-OD-PT-TEMP-1P50/` | 同上 |
...
| Run ID | 依赖 | 状态 | 参数/数据 | 命令 | 输出 | S/Q 验收 |
| --- | --- | --- | --- | --- | --- | --- |
| P1-OD-MOTION-SUMMARY-001 | 四个 P0-B reports PASS | TODO | stationary/constant/turning，既有 offsets 0/20/50/100/150 | 计划 §16.4，纯解析不推理 | `summary/p1_offline_diagnostic/motion_v1_v2_by_offset.{json,csv,md}` | S：可解析；Q：V1/V2机制与turning voxel效果均如实报告 |
| P1-OD-VX-STA-DEFAULT | SETUP | TODO | stationary；base,temp1,offsets 0/150 | §16.5 voxel command | `.../P1-OD-VX-STA-DEFAULT/` | S：六组齐；Q：静止offset对照 |
| P1-OD-VX-CV-DEFAULT | SETUP | TODO | constant；base,temp1,offsets 0/150 | 同上 | `.../P1-OD-VX-CV-DEFAULT/` | S：六组齐；Q：恒速motion体素对照 |
| P1-OD-VX-RANGE-30 | P1-OD-PT-RANGE-30结构有效 | TODO | stationary；range30,temp1,offset0 | 同上 | `.../P1-OD-VX-RANGE-30/` | S：六组；Q：coverage/accuracy/gap/AUROC delta |
| P1-OD-VX-DENSITY-4 | P1-OD-PT-DENSITY-4结构有效 | TODO | stationary；density4,temp1,offset0 | 同上 | `.../P1-OD-VX-DENSITY-4/` | 同上；非物理density claim |
| P1-OD-VX-VIEW-0P4 | P1-OD-PT-VIEW-0P4结构有效 | TODO | stationary；view.4,temp1,offset0 | 同上 | `.../P1-OD-VX-VIEW-0P4/` | 同上 |
| P1-OD-VX-SEMFLOOR-0P30 | P1-OD-PT-SEMFLOOR-0P30结构有效 | TODO | stationary；floor.30,temp1,offset0 | 同上 | `.../P1-OD-VX-SEMFLOOR-0P30/` | 同上 |
| P1-OD-VX-TEMP-0P75 | P1-OD-PT-TEMP-0P75结构有效 | TODO | stationary；base,temp.75,offset0 | 同上 | `.../P1-OD-VX-TEMP-0P75/` | 同上 |
| P1-OD-SUMMARY-001 | 所有已完成结构有效 P1-OD runs；允许Q-FAIL | TODO | 全部正/负结果 | 计划 §16.6 | `summary/p1_offline_diagnostic/{run_metrics.csv,ofat_point_comparison.json,voxel_comparison.json,P1_OFFLINE_DIAGNOSTIC_REPORT.md}` | S：strict汇总；Q：只给single-seed diagnostic排序 |

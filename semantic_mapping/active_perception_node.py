...
class ActivePerceptionNode(Node):
    def __init__(self):
        ...
        self.create_subscription(PointCloud2, '/voxel_entropy_data', self.entropy_cb, 10)
        ...
        # === 发布器 ===
        self.cmd_pub = self.create_publisher(Twist, '/cmd_vel', 10)            # 发给真机/仿真的最终速度
        self.entropy_pub = self.create_publisher(Float32, '/path_entropy', 10) # 监控当前路径的平均熵
        self.mode_pub = self.create_publisher(String, '/perception_mode', 10)  # 状态机：CONFIDENT/EXPLORING

        self.get_logger().info("🔥 主动感知降速节点 (Active Perception) 已完美启动！")

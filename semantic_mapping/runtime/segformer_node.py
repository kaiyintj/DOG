...
class SegformerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('posterior_temperature', 1.0)
        self.declare_parameter(
            'require_electric_bicycle_training_acceptance', True)
        self.declare_parameter(
            'electric_bicycle_min_target_iou', DEFAULT_MIN_TARGET_IOU)
        self.declare_parameter(
            'electric_bicycle_min_iou', DEFAULT_MIN_ELECTRIC_BICYCLE_IOU)
        self.declare_parameter(
            'electric_bicycle_min_road_iou', DEFAULT_MIN_ROAD_IOU)
        ...
        self.posterior_temperature = float(
            self.get_parameter('posterior_temperature').value)
        if not np.isfinite(self.posterior_temperature) or self.posterior_temperature <= 0.0:
            raise ValueError('posterior_temperature must be finite and positive')
        self.require_electric_bicycle_training_acceptance = bool(
            self.get_parameter(
                'require_electric_bicycle_training_acceptance').value)
        self.electric_bicycle_min_target_iou = float(np.clip(
            self.get_parameter('electric_bicycle_min_target_iou').value,
            0.0,
            1.0,
        ))
        self.electric_bicycle_min_iou = float(np.clip(
            self.get_parameter('electric_bicycle_min_iou').value,
            0.0,
            1.0,
        ))
        self.electric_bicycle_min_road_iou = float(np.clip(
            self.get_parameter('electric_bicycle_min_road_iou').value,
            0.0,
            1.0,
        ))
        ...
        self.project_posterior_topic = self.get_parameter(
            'project_posterior_topic').value
        ...
        if 'electric_bicycle' in self.supported_project_classes:
            ...
            accepted, metric_failures = training_report_meets_thresholds(
                training_report,
                min_target_iou=self.electric_bicycle_min_target_iou,
                min_electric_bicycle_iou=self.electric_bicycle_min_iou,
                min_road_iou=self.electric_bicycle_min_road_iou,
            )
            integrity_verified, integrity_detail = (
                verify_local_training_checkpoint(
                    self.model_id, training_report))
            if accepted and integrity_verified:
                self.electric_bicycle_training_report = str(report_path)
                self.get_logger().info(
                    'Accepted electric-bicycle training report: '
                    f'{report_path}, checkpoint_sha256={integrity_detail}')
            elif self.require_electric_bicycle_training_acceptance:
                raise RuntimeError(
                    'The configured checkpoint exposes electric_bicycle but '
                    'does not satisfy the runtime metric and checkpoint '
                    'integrity gates. '
                    f'failures={metric_failures}, integrity='
                    f'{integrity_detail}, report={report_path}. '
                    'Refusing navigation inference with an unverified class '
                    'head. Use segformer_checkpoint for diagnostics.')
            else:
                self.get_logger().warn(
                    'electric_bicycle is present in id2label but training '
                    'metrics were not accepted; this override is for '
                    'diagnostics only and must not be used for navigation.')
        ...
        self.project_posterior_pub = self.create_publisher(
            Image, self.project_posterior_topic, qos_profile_sensor_data)
        ...
        self.get_logger().info(
            'SegFormer checkpoint supported navigation classes: '
            + ', '.join(self.supported_project_classes))
        if unsupported_project_classes:
            self.get_logger().warn(
                'SegFormer checkpoint cannot emit navigation classes: '
                + ', '.join(unsupported_project_classes)
                + '. Queries for these classes will fail closed until a '
                'checkpoint with matching id2label entries is configured.')
        processor_summary = (
        ...
        self.get_logger().info(
            ...
            'raw logits are temperature-scaled and softmaxed at decoder '
            'resolution, then summed into the project ontology before any '
        ...
        self.get_logger().info(
            ...
            f'posterior_topic={self.project_posterior_topic}')
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            posterior = project_native_probabilities[0].permute(
                1, 2, 0).cpu().numpy()
            posterior_msg = posterior_array_to_image(posterior, msg.header)
            self.project_posterior_pub.publish(posterior_msg)
